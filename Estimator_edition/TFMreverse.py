...
class TFMBeam(NewsBeamsearcher):
    ...
    def get_input_fn(self):
        ...
        def input_fn():
            return tf.data.Dataset.from_generator(generator=data_generator,output_types=({'source':tf.int64,'context':tf.int64},tf.int64),output_shapes=({'source':[INPUT_LENGTH],'context':[OUTPUT_LENGTH]},[])).batch(self.topk)
        return input_fn
...
def beamsearch(topk,cnt,name,rp_fun = 'n'):
    ...
    bs.do_search_mt(OUTPUT_LENGTH,estimator=estimator,rp_fun=rp_fun)
    bs.report(name)
